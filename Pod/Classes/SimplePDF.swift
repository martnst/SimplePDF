...
open class SimplePDF {
    ...
    private class PDFWriter {
        ...
        private var currentLocation = CGPoint(x: CGFloat.greatestFiniteMagnitude, y: CGFloat.greatestFiniteMagnitude)
        ...
        fileprivate func drawHeaderFooterText(_ framesetter: CTFramesetter, textRect: CGRect) {
        ...
        fileprivate func addHeadline(_ string: String, style: TextStyle, backgroundBoxColor: UIColor? = nil, calculationOnly: Bool = false) -> NSRange {
            let backgroundBoxColor = backgroundBoxColor
        ...
        fileprivate func addImages(_ imagePaths:[String], imageCaptions: [String], imagesPerRow:Int = 3, spacing:CGFloat = 2, padding:CGFloat = 5, calculationOnly: Bool = false) -> NSRange {
        ...
        // MARK: - Drawing
        private func drawRect(_ rect: CGRect, fillColor: UIColor? = nil) {
        ...
        private func drawLine(_ p1: CGPoint, p2:CGPoint, color: UIColor? = nil, strokeWidth: CGFloat = 0.5) {
            var color = color
            if(color == nil) {
        ...
        private func convertRectToCoreTextCoordinates(_ r: CGRect) -> CGRect {
            var r = r
            let bounds = getPageBounds()
        ...
        private func convertPointToCoreTextCoordinates(_ p: CGPoint) -> CGPoint {
            var p = p
            let bounds = getPageBounds()
    ...
    @available(*, deprecated, message: "use pdfFileUrl instead")
    open var pdfFilePath: String {
        return pdfFileUrl.absoluteString
    }
    open let pdfFileUrl : URL
    ...
    public init(pdfTitle: String, authorName: String, fileName : String = "SimplePDF.pdf", replaceFileIfExisting: Bool = false, pageSize: PageSize = .a4, pageOrientation: PageOrientation = .portrait,
        leftMargin:CGFloat = 36, rightMargin:CGFloat = 36, topMargin: CGFloat = 72, bottomMargin: CGFloat = 36, textFormatter: DefaultTextFormatter = DefaultTextFormatter()) {
            ...
            self.pdfFileUrl = replaceFileIfExisting ? tmpFilePath : SimplePDFUtilities.renameFilePathToPreventNameCollissions(tmpFilePath)
